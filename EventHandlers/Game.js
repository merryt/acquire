    ...
    {
        {
            ...
            while (tilesForPlayer < 7) {
                ...
                if (tiles[randomRow][randomColumn] == 'B') {
                    if (tilesForPlayer == 0)
                        tiles[randomRow][randomColumn] = "X";
                    else
                        tiles[randomRow][randomColumn] = playerNumber + "";
                    tilesForPlayer++;
                }
            }
        }
    ...
    for (let playerNumber = 1; playerNumber <= numberOfPlayers; playerNumber++) {
            this.io.on('connection', function (socket) {
                ...
                for (var i = 1; i <= 9; i++)
                    for (var j = 1; j <= 12; j++) {
                        //activeGame.tiles.push(tiles[i][j]);
                        if (tiles[i][j] == playerNumber.toString())
                            tray += j + rowLabel(i);
                    }
                socket.emit('log', { message: 'Here are your tiles player' + playerNumber+" "+tray, id: socket.id });
            });
    }
